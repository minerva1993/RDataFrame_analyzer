...
def run_ana(tuple_args):
  ...
  #else: d = d.Define('EventWeight', wrongPVrate(folder_to_process, year))
  else: d = d.Define('EventWeight', 'genweight')
    

  #Jec/jer
  if isData: d = d.Define('jet_pt_jerc', 'jet_pt')
  else: d = d.Define('jet_pt_jerc', jec_str)
  ...
  for l_key, l_value in lepton_dict.items():
    hist_dict = histos(l_key, 0, postfix + syst_int)
  ...
  cut_dict = cut_flow(b_tagging(year))
  for c_key, c_value in cut_dict.items():

    lepton_dict = lepton_sel(year)
    for l_key, l_value in lepton_dict.items():
      hist_dict = histos(l_key, c_key, syst_int)

      for h_key, h_value in hist_dict.items():
        h = d.Filter(l_value + c_value, c_key).Histo1D(h_value, h_key, 'EventWeight')
        hists.append(h)
